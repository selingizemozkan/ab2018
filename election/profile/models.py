...
#hazır olan kısmı ezme işlemi
class UserProfileManager(BaseUserManager):
    use_in_migrations = True

    def _create_user(self, email, password, is_staff, is_superuser, **extra_fields):
        if not email:
            raise ValueError('The given username must be set') #emailin alınıp alınmadıgını kontrol ediyorum

        user = self.model(email=email, is_staff=is_staff, is_active=True,
                          is_superuser=is_superuser, **extra_fields)
        user.set_password(password) #parolayı saklayarak tutar
        user.save(using=self._db)
        return user

#ön tanımlı 2 tane fonk yazdık
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    USERNAME_FIELD = 'email' #kullanıcı login oldugunda email üzr karar verilir
